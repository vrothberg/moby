...
var (
	// DefaultRegistryEndpoint for docker.io
	DefaultRegistryEndpoint = RegistryEndpoint{
...
type Registry struct {
	...
	Pull []RegistryEndpoint `json:"pull,omitempty"`
	// Push is a slice of registries serving as push endpoints.
	Push []RegistryEndpoint `json:"push,omitempty"`
	// prefixLength is the length of prefix and avoids redundant length
	// calculations.
	prefixLength int
}

// RegistryEndpoint includes all data associated with a given registry endpoint.
type RegistryEndpoint struct {
	...
	// Address when the RegistryEndpoint is used.
	url url.URL
...
func RewriteReference(ref reference.Named, prefix string, registry *url.URL) (reference.Named, error) {
	...
	if *registry == DefaultRegistryEndpoint.url {
		return ref, nil
	}
...
// GetURL returns the RegistryEndpoint's URL.
func (r *RegistryEndpoint) GetURL() *url.URL {
...
// Prepare sets up the RegistryEndpoint.
func (r *RegistryEndpoint) Prepare() error {
...
func (r *Registry) Prepare() error {
	...
	prepareEndpoints := func(endpoints []RegistryEndpoint) ([]RegistryEndpoint, error) {
		...
		for i := range endpoints {
			...
			if official && addDefaultEndpoint {
				if endpoints[i].Address == DefaultRegistryEndpoint.Address {
		...
		if addDefaultEndpoint {
			r.Pull = append(endpoints, DefaultRegistryEndpoint)
		}
